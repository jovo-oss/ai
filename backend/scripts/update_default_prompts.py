...
def update_default_prompt_templates():
    """更新默认提示词模板"""
    db = SessionLocal()
    
    try:
        # 删除所有旧的默认模板
        old_templates = db.query(PromptTemplate).filter(
        ...
        db.commit()
        print(f"成功添加 {len(default_templates)} 个新的默认提示词模板")
        
    except Exception as e:
        db.rollback()
        print(f"更新默认提示词模板失败: {e}")
    finally:
        db.close()

if __name__ == "__main__":
    update_default_prompt_templates()
